...
def test_literals():
    ...
    assert TestParsers.ab.parse("abb") == Failure(
        ParseError(StringReader("abb", 2), ["end of source"])
    )
...
def test_predicate():
    ...
    assert TestParsers.d.parse("23") == Failure(
        ParseError(StringReader("23", 1), ["end of source"])
    )
...
def test_optional():
    ...
    assert TestParsers.b.parse("c") == Failure(
        ParseError(StringReader("c", 0), ["'a'", "end of source"])
    )
...
def test_optional_longer():
    ...
    assert TestParsers.b.parse("ac") == Failure(
        ParseError(StringReader("ac", 0), ["'ab'", "end of source"])
    )
    assert str(TestParsers.b) == "b = opt(a)"
...
def test_alternative():
    ...
    assert TestParsers.bc.parse("ce") == Failure(
        ParseError(StringReader("ce", 0), ["'b'", "'cd'"])
    )
...
def test_multiple():
    ...
    for parser in [TestParsers.back, TestParsers.front, TestParsers.both]:
        ...
        assert parser.parse("bbc") == Failure(
            ParseError(StringReader("bbc", 0), ["'aaaa'", "'bbb'", "'cc'", "'d'"])
        )
        assert parser.parse("bbba") == Failure(
            ParseError(StringReader("bbba", 3), ["end of source"])
        )
...
def test_repeated():
    ...
    assert TestParsers.bs.parse("bbbc") == Failure(
        ParseError(StringReader("bbbc", 3), ["'b'", "end of source"])
    )
    assert TestParsers.cs.parse("ccc") == Success(["c", "c", "c"])
    assert TestParsers.cs.parse("c") == Success(["c"])
    assert TestParsers.cs.parse("") == Success([])
    assert TestParsers.cs.parse("cccb") == Failure(
        ParseError(StringReader("cccb", 3), ["'c'", "end of source"])
    )
...
def test_repeated_longer():
    ...
    assert TestParsers.bf.parse("bfbc") == Failure(
        ParseError(StringReader("bfbc", 2), ["'bf'", "end of source"])
    )
    ...
    assert TestParsers.cf.parse("cfcb") == Failure(
        ParseError(StringReader("cfcb", 2), ["'cf'", "end of source"])
    )
...
def test_eof_protection():
    ...
    assert TestParsers.bba.parse("ab") == Failure(
        ParseError(StringReader("ab", 1), ["end of source"])
    )
...
def test_success_failure_protection():
    ...
    assert TestParsers.bbb.parse("aabb") == Failure(
        ParseError(StringReader("aabb", 2), ["something else"])
    )
